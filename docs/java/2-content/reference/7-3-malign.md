...
``` kotlin
malign = PowerTurret("malign").apply {
    requirements(Category.turret, ItemStack.with(Items.carbide, 200, Items.beryllium, 1000, Items.silicon, 500, Items.graphite, 500, Items.phaseFabric, 200))

    val haloProgress = PartProgress.warmup
    val haloColor = Color.valueOf("d370d3")
    val heatCol = Color.purple
    val haloY = -15f
    val haloRotSpeed = 1.5f

    val circleProgress = PartProgress.warmup.delay(0.9f)
    val circleColor = haloColor
    val circleY = 25f
    val circleRad = 11f
    val circleRotSpeed = 3.5f
    val circleStroke = 1.6f

    shootSound = Sounds.shootMalign
    loopSound = Sounds.loopMalign
    loopSoundVolume = 1.3f

    shootType = FlakBulletType(8f, 70f).apply {
        sprite = "missile-large"
        ...
        fragBullet = LaserBulletType(65f).apply {
            colors = arrayOf(haloColor.cpy().a(0.4f), haloColor, Color.white)
            buildingDamageMultiplier = 0.25f
            width = 19f
            hitEffect = Fx.hitLancer
            sideAngle = 175f
    ...
    drawer = DrawTurret("reinforced-").apply {
        parts.addAll(
            ...
            //actual turret
            RegionPart("-mouth").apply {
                heatColor = heatCol
                heatProgress = PartProgress.warmup

                moveY = -8f
            },
            RegionPart("-end").apply {
                moveY = 0f
            },

            RegionPart("-front").apply {
                heatColor = heatCol
                heatProgress = PartProgress.warmup
        ...
        val heatCol2 = heatCol.cpy().add(0.1f, 0.1f, 0.1f).mul(1.2f)
        for (i in 1 until 4) {
            val fi = i
            parts.add(RegionPart("-spine").apply {
                outline = false
                progress = PartProgress.warmup.delay(fi / 5f)
                heatProgress = PartProgress.warmup.add { p -> (Mathf.absin(3f, 0.2f) - 0.2f) * p.warmup }
                mirror = true
                ...
                color = Color.valueOf("bb68c3")
                heatColor = heatCol2
                moves.add(PartMove(PartProgress.recoil.delay(fi / 5f), 1f, 0f, 3f))
            })
        }
    }
    ...
    consumePower(40f)
    unitSort = UnitSorts.strongest
    shoot = ShootSummon(0f, 0f, circleRad, 20f)
