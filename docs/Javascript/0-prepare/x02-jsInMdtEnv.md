...
注意:第二块的写法和第一块的写法是完全等价的.

也就是说,解构得到的变量和直接从模块中获取的对象的值是相同的,但并非完全等价.解构得到的变量是一个新的变量,它们和原来的对象没有直接的联系.如果你修改了解构得到的变量,并不会影响原来的对象,反之亦然.
...
> 我被这个坑过,如果你想要修改原来的对象,就不要使用解构.

而且解构有时也可能会让代码变得不太清晰,尤其是当你从一个模块中提取了很多对象的时候,这时候你就需要权衡一下是否使用解构了.
